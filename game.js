...
function init() {
    ...
    if (sndSwitch) {
        ...
        sndSwitch.addEventListener('change', () => {
            ...
            if (soundEnabled && clickAudio) { clickAudio.currentTime = 0; clickAudio.play().catch(()=>{}); }
        });
    }
...
function startGame() {
    ...
    CONFIG.gameLoop = setInterval(gameUpdate, CONFIG.currentSpeed);
...
function restartGame() {
    const margin = 8;
    const startX = margin + Math.floor(Math.random() * (CONFIG.tileCountX - margin * 2));
    const startY = margin + Math.floor(Math.random() * (CONFIG.tileCountY - margin * 2));
...
function gameUpdate() {
    ...
    // (Suppression de l'assistance ralentissement mur)
    ...
    if (head.x < 0 || head.x >= CONFIG.tileCountX || 
        head.y < 0 || head.y >= CONFIG.tileCountY) {
        if (!snake.wallGraceActive) {
            snake.wallGraceActive = true;
            snake.wallGraceTimeout = setTimeout(() => {
                // Si le joueur n'a pas tourné, game over
                if (snake.wallGraceActive) {
                    gameOver();
    ...
    if (head.x === food.position.x && head.y === food.position.y) {
        ...
        if (newLevel > CONFIG.level) {
            ...
            CONFIG.currentSpeed = Math.max(30, CONFIG.baseSpeed - (CONFIG.level * diff.speedIncrease));
            CONFIG.gameLoop = setInterval(gameUpdate, CONFIG.currentSpeed);
        }
    } else {
        snake.body.pop();
    }
    // keep logic separate from render; render loop will draw and update particles
}
...
// Continuous render for smooth visuals
...
function handleKeyPress(e) {
    ...
    const dangerZoneY = 5;
    const nearWall = head && (
        head.x < dangerZoneX || head.x >= CONFIG.tileCountX - dangerZoneX ||
        head.y < dangerZoneY || head.y >= CONFIG.tileCountY - dangerZoneY
    );
    const maxQueue = nearWall ? 4 : 2;

    // Check against the last direction in queue OR nextDirection if queue is empty
    const lastDir = inputQueue.length > 0 ? inputQueue[inputQueue.length - 1] : snake.nextDirection;
